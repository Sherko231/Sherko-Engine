...
Phase 1 and Phase 2 are complete. P3-T01 / Issue #84 through P3-T06 / Issue #89 are formally complete. P3-T04A / #149 established `game-sandbox` as the canonical owner-facing manual demo while preserving the headless server boundary, and P3-T04B / #151 completed the sandbox logging cleanup.
...
P3-T06 / Issue #89 completed through PR #156 / merged `master` `92ad157adb696bd5a9d21933f6d9af60be3634a7`; final heavy workflow #285 and exact-merge lightweight workflow #286 passed. The public platform boundary now exposes `InputSnapshot`, `InputKey`, `InputMouseButton`, and `GlfwWindow.captureInputSnapshot(long)` without exposing GLFW/LWJGL types.

P3-T07 / Issue #90 is the active Phase 3 task on branch `p3-t07-action-bindings`. It adds strict versioned JSON v1 loading for exactly eleven gameplay actions through immutable, device-neutral binding metadata. `MOVE` and `LOOK` are vector actions; the remaining actions are digital. The loader reuses the P3-T06 key/button vocabulary plus relative mouse axes, keeps Jackson as an implementation-only dependency, and does not implement P3-T08 action evaluation/transitions or P3-T09 tick/replay commands.

The sandbox remains unchanged for P3-T07 because configuration metadata is not usefully human-observable without the later P3-T08 evaluation layer. Existing P3-T06 snapshot diagnostics continue to use only authorized public production APIs.
