...
Phase 5 — Rendering foundation is now the current M2 focus. P5-T01 through P5-T17 are accepted, including the bounded public `OpenGlRenderer` indexed-draw path, window-owned presentation, explicit D-056 sRGB texture/default-framebuffer color path, D-057 internal material/state policy, D-058 immutable per-frame `RenderFramePacket` submission boundary, D-059 bounded CPU frustum culling with latest-success diagnostics, D-060 deterministic opaque/transparent draw ordering, D-061 one bounded renderer-owned unshadowed directional light, D-062 bounded public point/spot submissions with a configurable 1–8 local-light maximum and deterministic first-N overflow handling, D-063 one explicit internal presentation mode that preserves linear shading and guarantees exactly one IEC sRGB output encode, D-064 bounded renderer-neutral per-frame debug line/AABB/sphere/ray submissions plus latest-success text counters, and D-065 one internal first-person view-model validation layer with independent projection and isolated depth. P5-T07A / Issue #213 repaired the renderer's public Gradle/API boundary without changing public signatures. P3-T04C / Issue #216, P5-T03A / Issue #221, P5-T07B / Issue #224, and P5-T15A / Issue #243 are accepted maintenance repairs. P5-T01 through P5-T17 are accepted, but Phase 5 is not yet complete: exit review #250 found the integrated sandbox lacked a real textured room and camera movement affecting the rendered view. P5-T18 / #251 is now the active bounded repair on its dedicated branch, limited to the fixed internal textured-room fixture plus owner-controlled rendered camera; Phase 6 remains inactive until P5-T18 is accepted and #250 is rerun successfully. The roadmap keeps 1080p60 as the initial performance target, but exact minimum CPU/GPU/driver/RAM/VRAM qualification is not a current gate and must not be claimed without separate future evidence.
