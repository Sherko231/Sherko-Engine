...
Phase 5 — Rendering foundation is now the current M2 focus. P5-T01 through P5-T16 are accepted, including the bounded public `OpenGlRenderer` indexed-draw path, window-owned presentation, explicit D-056 sRGB texture/default-framebuffer color path, D-057 internal material/state policy, D-058 immutable per-frame `RenderFramePacket` submission boundary, D-059 bounded CPU frustum culling with latest-success diagnostics, D-060 deterministic opaque/transparent draw ordering, D-061 one bounded renderer-owned unshadowed directional light, D-062 bounded public point/spot submissions with a configurable 1–8 local-light maximum and deterministic first-N overflow handling, D-063 one explicit internal presentation mode that preserves linear shading and guarantees exactly one IEC sRGB output encode, and D-064 bounded renderer-neutral per-frame debug line/AABB/sphere/ray submissions plus latest-success text counters. P5-T07A / Issue #213 repaired the renderer's public Gradle/API boundary without changing public signatures. P3-T04C / Issue #216, P5-T03A / Issue #221, P5-T07B / Issue #224, and P5-T15A / Issue #243 are accepted maintenance repairs. P5-T17 / Issue #199 is active on `p5-t17-view-model-layer`, adding one bounded internal first-person view-model validation layer with its own projection/depth policy and engine-owned fixture while preserving D-041/D-045 world-camera semantics and deferring all asset/gameplay/HUD scope. The roadmap keeps 1080p60 as the initial performance target, but exact minimum CPU/GPU/driver/RAM/VRAM qualification is not a current gate and must not be claimed without separate future evidence.
