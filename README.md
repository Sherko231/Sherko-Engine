...
Phase 5 — Rendering foundation is now the current M2 focus. P5-T01 through P5-T15 are accepted, including the bounded public `OpenGlRenderer` indexed-draw path, window-owned presentation, explicit D-056 sRGB texture/default-framebuffer color path, D-057 internal material/state policy, D-058 immutable per-frame `RenderFramePacket` submission boundary, D-059 bounded CPU frustum culling with latest-success debug counters, D-060 deterministic opaque/transparent draw ordering, D-061 one bounded renderer-owned unshadowed directional light, D-062 bounded public point/spot submissions with a configurable 1–8 local-light maximum and deterministic first-N overflow handling, and D-063 one explicit internal presentation mode that preserves linear shading and guarantees exactly one IEC sRGB output encode. P5-T07A / Issue #213 repaired the renderer's public Gradle/API boundary without changing public signatures. P3-T04C / Issue #216, P5-T03A / Issue #221, and P5-T07B / Issue #224 are accepted maintenance repairs. P5-T16 / Issue #198 is active on `p5-t16-debug-geometry`, adding bounded renderer-neutral line/AABB/sphere/ray debug submissions plus bounded text counters, with OpenGL adaptation kept internal and no editor/runtime-UI/physics/network implementation pulled forward. The roadmap keeps 1080p60 as the initial performance target, but exact minimum CPU/GPU/driver/RAM/VRAM qualification is not a current gate and must not be claimed without separate future evidence.
