...
Phase 5 — Rendering foundation is now the current M2 focus. P5-T01 through P5-T07 are accepted, including the first bounded public `OpenGlRenderer` indexed-draw path and window-owned presentation. P5-T08 / Issue #190 is the next planned task and is not active until deliberately activated against current repository state. The roadmap keeps 1080p60 as the initial performance target, but exact minimum CPU/GPU/driver/RAM/VRAM qualification is not a current gate and must not be claimed without separate future evidence.
