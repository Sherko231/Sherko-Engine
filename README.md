...
Phase 4 is the active M1 focus. P4-T01 / Issue #94 is accepted and establishes the canonical world convention in [`docs/SPATIAL_CONVENTIONS.md`](docs/SPATIAL_CONVENTIONS.md): right-handed world, +X right, +Y up, -Z forward, meters for linear world quantities, radians for internal angular quantities, right-hand-rule positive rotation, and dimensionless scale. P4-T02 / Issue #95 is accepted and pins/verifies JOML hot-loop use. P4-T03 / Issue #96 is the active executable task adding cached hierarchical `Transform` composition and promoting JOML to the public `engine-core` spatial API. P4-T04 remains the next planned task for explicit transform-cycle rejection.
