...
Phase 5 — Rendering foundation is now the current M2 focus. P5-T01 through P5-T12 are accepted, including the bounded public `OpenGlRenderer` indexed-draw path, window-owned presentation, explicit D-056 sRGB color path, D-057 internal material/state policy, D-058 immutable per-frame `RenderFramePacket` submission boundary, D-059 bounded CPU frustum culling with latest-success debug counters, and D-060 deterministic opaque/transparent draw ordering. P5-T07A / Issue #213 repaired the renderer's public Gradle/API boundary without changing public signatures. P3-T04C / Issue #216, P5-T03A / Issue #221, and P5-T07B / Issue #224 are accepted maintenance repairs. P5-T13 / Issue #195 is active on `p5-t13-directional-light`, adding one bounded renderer-owned unshadowed directional light with explicit D-041 world-direction and linear-color semantics while preserving the existing public renderer/submission boundaries. The roadmap keeps 1080p60 as the initial performance target, but exact minimum CPU/GPU/driver/RAM/VRAM qualification is not a current gate and must not be claimed without separate future evidence.
