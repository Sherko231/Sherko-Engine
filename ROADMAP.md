...
P5R-T01 through P5R-T04 are accepted, establishing the Phase 5R naming/refactor standard, repository-wide symbol inventory, public/internal/package boundary audit, native/backend decomposition, and input/focus/cursor decomposition behind the unchanged `GlfwWindow` facade. **P5R-T05** is the active bounded window-mode/size decomposition candidate; P5R-T06 must not begin until T05 is accepted. P6-T01 remains blocked. Only after the Phase 5R exit review passes may P6-T01 be materialized. Exact task definitions and acceptance criteria remain in the [technical backlog](docs/roadmap/TECHNICAL_BACKLOG.md).
