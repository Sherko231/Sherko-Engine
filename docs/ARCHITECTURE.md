...
`engine-assets` now owns package-private AUDIO validation for case-insensitive `.ogg` Ogg Vorbis sources through the existing LWJGL 3.4.3 stb binding. The importer reads source bytes into Java-owned storage, copies them to explicitly freed native memory, and calls `stb_vorbis_decode_memory` so the complete in-memory stream is decoded before output creation. The returned metadata must report one or two channels and a positive sample rate. The temporary decoded PCM is freed immediately in a `finally` path and never becomes persisted or public state.
