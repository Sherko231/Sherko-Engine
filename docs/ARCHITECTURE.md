...
P5-T09 / D-057 keeps the first material model internal to `engine-render-opengl` until P6 defines stable runtime asset/resource references. One immutable `RendererMaterial` carries an explicit shader variant, immutable texture/sampler bindings, bounded linear scalar color multipliers, blend mode, depth mode, and cull mode without owning any native resource. The fixed reference scene reuses the same P5-T07 indexed mesh and P5-T08 texture/program twice: a left baseline material preserves the neutral-gray sRGB regression, while a right tinted material uses distinct material data and render state. Material state selects fixed OpenGL blend/depth/cull policy instead of entity-type branching; the render path restores the full framebuffer viewport and existing program/VAO/texture/framebuffer-sRGB cleanup boundaries. No public material API, P6 asset ID/handle, P5-T10 submission packet, lighting, or gameplay/world dependency is introduced.
...
| `engine-render-opengl` | OpenGL renderer and runtime-UI draw adapter | Bounded public indexed-mesh renderer plus P5-T03..T11 resource/upload/shader/uniform/sRGB/material/frame-snapshot/CPU-culling foundations | `engine-core`, `engine-platform-lwjgl`, `engine-assets`, `engine-ui` |
