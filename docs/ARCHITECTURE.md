...
`engine-assets` now owns package-private AUDIO validation for case-insensitive `.ogg` Ogg Vorbis sources through the existing LWJGL 3.4.3 stb binding. The importer reads source bytes into Java-owned storage, copies them to explicitly freed native memory for `stb_vorbis_open_memory`, reads decoder metadata, accepts only one or two channels with a positive sample rate, and drains decoded samples to end-of-stream so malformed/truncated data fails before the output cache is created. Every non-null stb_vorbis handle is closed in a `finally` path.
