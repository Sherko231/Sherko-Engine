...
P5-T08 / D-056 adds the first explicit renderer color-space path without changing the public renderer signature. `GlfwWindow` requests an sRGB-capable default framebuffer. Renderer-internal texture encoding distinguishes display-referred color (`GL_SRGB8_ALPHA8`) from linear data (`GL_RGBA8`); the fixed production triangle owns one 1x1 neutral-gray sRGB texture and internal sampler, and its fragment shader samples that texture in linear space. `OpenGlRenderer.render(...)` enables `GL_FRAMEBUFFER_SRGB` only for the frame clear/draw section and disables it in cleanup, producing exactly one hardware linear-to-sRGB encode at the default framebuffer boundary. No public texture/material API, asset decoding/cooking, mip generation, HDR, tonemapping, fog, post-processing, or P5-T15 presentation layer is introduced.
