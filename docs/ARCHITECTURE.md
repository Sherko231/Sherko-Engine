...
`game-sandbox` is the canonical owner-facing manual demo surface. `EngineDemoMain` composes only public production APIs: `GlfwWindow`, logical/framebuffer size delivery, window-mode transitions, cursor capture/focus-loss behavior, P3-T06 `InputSnapshot`/`InputKey`, P3-T07 `InputActionBindings`, P3-T08 `InputActionEvaluator`/action snapshots, P3-T09 `PlayerInputCommandSampler`/core tick commands, `EngineClock`, `FixedStepAccumulator`, `FixedStepCatchUpPolicy`, structured logging, and `NativeResourceRegistry` verification. The scripted timeline lets the owner observe current engine behavior before renderer/gameplay layers exist.
...
P3-T06 captures one public `InputSnapshot` per demo frame after event polling. P3-T08 evaluates that snapshot through a committed demo schema-v1 binding set. P3-T09 submits the resulting action snapshot to the tick sampler and emits bounded tick-command diagnostics only when fixed-step timing reports actual due simulation ticks. P3-T10 does not add a settings UI/configuration surface, so no sandbox source change is required merely to demonstrate scalar response math; focused public-API tests own that acceptance.

The sandbox is not verification authority. Its timing/input/action/tick lines are diagnostic only and are explicitly not FPS or benchmark evidence. The current window remains visually empty until renderer work creates a public production presentation path. Unit/native/integration tests, exact final-candidate heavy CI, exact-merge lightweight master verification, and P0 feasibility gates remain separate acceptance evidence.
...
| Canonical world-space convention | P4-T01 / Issue #94: D-041 + `docs/SPATIAL_CONVENTIONS.md`; documentation/architecture contract, no runtime math API yet |
| Owner-facing sandbox demo | P3-T04A / Issue #149 plus P3-T06/P3-T08/P3-T09 diagnostics: public-API demo with non-exported platform runtime |
