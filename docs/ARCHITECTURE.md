...
Imported values are deliberately pre-engine-conversion: positions, normals, tangent xyz, UV0 xy, vertex order, face order, indices, and winding are preserved exactly as returned by Assimp. P6-T04 performs no left/right-handed conversion, axis swap/negation, unit scaling, UV flip, winding reversal, node pre-transform, generated normals/tangents, vertex joining, optimization, or other source-data rewrite. P6-T05 is the sole next boundary authorized to convert source coordinates/units into the D-041 engine convention exactly once.
