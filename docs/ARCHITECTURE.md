...
The persistent playground composes only public production APIs: `GlfwWindow`, `OpenGlRenderer`, `AssetLoaders`/`AssetLoader`, typed `ResourceHandle` values, backend-neutral `MeshAsset`/`MaterialAsset`, logical/framebuffer size delivery, window-owned `present()`, window-mode transitions, cursor capture/focus-loss behavior, P3-T06 `InputSnapshot`/`InputKey`, P3-T07 `InputActionBindings`, P3-T08 `InputActionEvaluator`/action snapshots, P3-T09 `PlayerInputCommandSampler`/core tick commands, P3-T10 `InputResponseSettings`, `CameraMatrices`, `EngineClock`, `FixedStepAccumulator`, `FixedStepCatchUpPolicy`, structured logging, and `NativeResourceRegistry` verification. It has no fixed duration or automatic feature tour; the owner keeps these capabilities active together and exits explicitly with `Ctrl+Q`.

Issue #408 adds the bounded Phase 6 Asset Lab. The sandbox materializes a committed cooked fixture into a temporary writable cache, opens it through public `AssetLoaders.open`, observes asynchronous MESH/MATERIAL handle state, demonstrates structured missing-content fallback and MATERIAL last-valid hot reload, and deletes the temporary cache best-effort on shutdown. READY material RGB values affect only already-public local-light submission colors as a visualization; this does not create arbitrary mesh/material renderer submission. Offline cooker/import/texture/audio/dependency-graph and renderer-internal upload/shader-reload behavior remains outside the sandbox.

Current owner controls are intentionally built from the existing public input vocabulary: `E` cycles a valid MATERIAL preset, `Right Shift + E` demonstrates failed reload preservation, `Alt + E` demonstrates missing-MESH fallback, `Ctrl + E` releases/reloads asset handles, `F` cycles window modes, `R` toggles cursor capture, `Right Shift + F` cycles mouse sensitivity, `Right Shift + R` toggles mouse-Y inversion, and `Ctrl+Q` exits. Normal non-reserved action bindings continue to evaluate concurrently. `SandboxControlsTest` verifies control selection without a native window, and `SandboxAssetLabTest` exercises the real public cooked-runtime loader without a native graphics window. The sandbox still makes no direct OpenGL/LWJGL call.
...
## P6-T09 offline Ogg Vorbis audio cooking

`engine-assets` now owns package-private AUDIO validation for case-insensitive `.ogg` Ogg Vorbis sources through the existing LWJGL 3.4.3 stb binding. The importer reads source bytes into Java-owned storage, copies them to explicitly freed native memory, opens the stream with `stb_vorbis_open_memory`, reads metadata with `stb_vorbis_get_info`, and drains interleaved samples through `stb_vorbis_get_samples_short_interleaved` until EOF before output creation. The metadata must report one or two channels and a positive sample rate, the terminal decoder error must remain `VORBIS__no_error`, and the decoder handle plus temporary validation buffer are released in `finally` paths. No native decoder state becomes persisted or public.

P6-T09 deliberately preserves the exact validated compressed Ogg Vorbis source payload instead of persisting PCM or transcoding. Runtime buffering/streaming policy remains deferred. AUDIO payloads use package-private little-endian `SAUD` schema v1: magic `SAUD`, schema version 1, codec id 1 for Ogg Vorbis, channel count, sample rate, exact compressed payload length, CRC32C, then the original compressed bytes. Decode validates the complete header, mono/stereo metadata, positive sample rate, exact payload length/checksum, truncation, and trailing data before returning Java-owned values. The cooked-format decoder itself invokes neither stb_vorbis nor OpenAL.

Manifest v1, AssetId-derived cooked paths, P6-T07 MESH `SMES`, P6-T08 TEXTURE `STEX`, and non-MESH/non-TEXTURE/non-AUDIO pass-through remain unchanged. No public runtime audio/resource API, OpenAL playback path, resampling, mixing, spatialization, loop metadata, dependency graph, or later Phase 6 resource system is introduced.


## P6-T10 offline asset dependency graph

`engine-assets` now owns package-private dependency authoring/validation in addition to the existing cooker formats. P6-T10 deliberately leaves D-068 `<source>.asset.json` schema v1 unchanged. An optional adjacent `<source>.deps.json` schema v1 is allowed only for MATERIAL, PREFAB, and SCENE source assets and contains exactly `schemaVersion`, `assetDependencies`, and `shaderDependencies`.

Asset dependency edges use canonical existing `AssetId` values. MATERIAL asset edges may target TEXTURE assets only; PREFAB/SCENE edges may target any existing source asset except themselves. MATERIAL may also declare canonical lowercase opaque logical shader keys matching `[a-z][a-z0-9._-]{0,127}`. These keys are dependency identifiers only: they are not source paths, AssetIds, renderer/native handles, a SHADER `AssetType`, or a public shader API. PREFAB/SCENE shader arrays must be empty. Duplicate/self/missing/type-invalid references, unsupported or orphan sidecars, and cycles fail before output-cache creation.
...
## P6-T13 asynchronous runtime mesh loading and render-thread upload

`engine-assets` now exposes public `MeshAsset`, `AssetLoader`, and `AssetLoaders`. Opening a loader validates cooker `manifest.json` schema v1 once, including exact entry fields, canonical AssetIds/types, positive integral byte sizes, duplicate rejection, and the canonical `assets/<AssetId>.bin` cooked path. Source paths remain authoring metadata and are not used to locate runtime content.

Manifest-backed MESH requests return a P6-T11 `ResourceHandle<MeshAsset>` in LOADING and schedule file read plus SMES checksum/format decode on Java virtual threads. SMES v1 is not compressed, so P6-T13 performs no synthetic decompression or persisted-format change; a future compressed cooked schema would place decompression in the same worker stage. Missing identities or disappeared cooked files reuse the P6-T12 fallback cube and complete READY with a MISSING_CONTENT diagnostic. Existing-entry I/O failures complete FAILED with READ_FAILED; manifest-size mismatch or invalid SMES completes FAILED with INVALID_CONTENT. Release during in-flight work remains terminal through package-private completion-if-loading semantics.

`engine-render-opengl` consumes only `com.samo.engine.assets.api` and adds package-private `RuntimeMeshGpuUploader`. It asserts the existing D-049 `OpenGlThreadGuard` before any resource backend mutation, packs decoded primitive attributes in the accepted SMES order, creates/allocates/uploads explicit OpenGL vertex and index buffers on the owner thread, and rolls back partially created buffers on failure. The uploaded value is internal and AutoCloseable; no native handle enters the public asset API.
