...
P3-T07 adds no project dependency edge. Jackson is a library implementation dependency of `engine-platform-lwjgl` only; public signatures must contain no Jackson class and `game-server` remains independent of the platform module.
...
P3-T06 captures one public `InputSnapshot` per demo frame after event polling and prints bounded once-per-second input diagnostics. P3-T07 intentionally does not add action diagnostics because binding metadata is not meaningful owner-observable behavior until P3-T08 evaluates action values. This keeps the sandbox from inventing future semantics.
...
| Data-driven gameplay action-binding metadata | P3-T07 / Issue #90: strict JSON-v1 loader, eleven typed actions, immutable descriptors, Jackson implementation dependency, and deterministic schema tests; final CI acceptance pending while active |
| Owner-facing sandbox demo | P3-T04A / Issue #149 plus P3-T06 diagnostics: public-API demo with non-exported platform runtime; P3-T07 correctly records no demo change until evaluation exists |
