...
P5-T09 / #191 is active on branch `p5-t09-material-record`. The candidate keeps `RendererMaterial` internal until P6 defines stable resource references, carries explicit shader/textures/scalars/blend/depth/cull data, and reuses the same fixed indexed mesh twice with a baseline and tinted material through the unchanged public `OpenGlRenderer.render(...)` call. No project dependency, production dependency, public signature, asset format, world/gameplay coupling, P5-T10 submission contract, or lighting behavior is added. Final acceptance still requires focused verification plus the normal exact-head five-job CI, merge, and exact-merge Lightweight verification.
...
Complete P5-T09 / #191 on branch `p5-t09-material-record`: finish the internal material/state implementation, deterministic and native two-material evidence, sandbox/wiki synchronization, consistency review, and required verification. Open the final non-draft PR only when the candidate is complete; merge only after all five exact-head jobs pass, then require exact-merge Lightweight verification before closing #191. Stop before P5-T10 submission packets, P6 public asset/resource handles, lighting, or public arbitrary material/texture APIs.
