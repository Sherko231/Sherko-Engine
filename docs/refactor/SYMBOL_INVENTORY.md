...
## P5R-T21 accepted public API naming audit
...
Accepted T21 selects **KEEP** for every currently supported public type/member. No additional rename, alias, wrapper, package move, visibility widening, or compatibility shim is justified. The prior T07 lifecycle coordinator renames remain the only public-name corrections introduced by Phase 5R so far.
...
Historical CI/evidence identifiers remain KEEP even when they reflect original roadmap vocabulary rather than later implementation names.
...
- renderer visual-demo types remain in the existing internal package because `MaterialComparisonOverlay` consumes package-private renderer infrastructure.
