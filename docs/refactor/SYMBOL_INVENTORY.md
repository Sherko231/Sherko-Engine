...
| `AnimatedDemoLighting` | `LightingFrame` | package-level nested record | KEEP | — | T18 candidate moves animated demo light/debug-marker generation into a dedicated package-private owner; the nested value names one generated frame. |
| `MaterialComparisonOverlay` | top-level visualDemo class | KEEP | — | T18 candidate moves the existing overlay into a package-private visual-demo GL resource owner without changing allocation, draw, rollback, or close semantics. |
