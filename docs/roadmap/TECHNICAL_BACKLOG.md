...
- [ ] P5R-T10 Rename/decompose the fixed reference-scene renderer path. Candidate: `IndexedStaticMeshPipeline` -> internal `ReferenceSceneRenderer`, with package-private `ReferenceRoomFixture` owning fixed room vertex/index/texture/bounds/count data only. Acceptance remains: reference-room/world responsibility is clear, fixture construction is separated, Phase 5 output/public renderer contracts are unchanged, and T11+ orchestration work is not pulled forward.
