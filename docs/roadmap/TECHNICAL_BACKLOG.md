...
- [ ] P6-T10 Build a dependency graph from materials to textures/shaders and scenes/prefabs to their assets. Acceptance: recooking a texture invalidates only affected dependents. Active executable contract: Issue #390 from `master` `e7012ceaeddb9526c1c8360f512b9d740e3114bb`; asset edges use existing AssetIds, material shader references use opaque logical shader keys without adding a SHADER AssetType, and exact reverse invalidation is persisted through `dependencies.json` while incremental cache mutation remains out of scope.
