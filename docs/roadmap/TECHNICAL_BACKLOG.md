...
- [ ] P5R-T01 Define the refactor/naming standard and produce a repository-wide symbol inventory with proposed keep/rename/decompose decisions. Acceptance: every production/runtime Java class plus relevant nested public/package-level types is classified as keep, rename, decompose, move, or remove with a reason; no Java behavior changes.
- [ ] P5R-T02 Audit public/internal/package boundaries before renaming. Acceptance: every public package/type touched by later P5R work has a documented consumer role, and no implementation type is promoted to public merely for convenience.
- [ ] P5R-T03 Decompose `GlfwWindow` native/backend plumbing while preserving `GlfwWindow` as the public platform facade. Acceptance: native callback/backend responsibilities move to descriptively named internal classes, all existing lifecycle/window/native tests remain behaviorally unchanged, and no public signature/module edge changes.
- [ ] P5R-T04 Decompose `GlfwWindow` input/focus/cursor responsibilities. Acceptance: focus safety, held/edge state, cursor capture, and relative/raw mouse tracking have clear single-purpose internal owners and all P3 focus/input regressions pass unchanged.
- [ ] P5R-T05 Decompose `GlfwWindow` window-mode/size transition responsibilities. Acceptance: windowed/borderless/exclusive transition planning, monitor targeting, geometry restore, and deferred size delivery are owned by clearly named internal types with existing P3 window-mode behavior unchanged.
- [ ] P5R-T06 Refactor input-binding loading/parsing/validation names and responsibilities. Acceptance: JSON parsing and semantic validation responsibilities are clear from class/method names, schema v1 and error behavior remain byte/meaning compatible, and input tests stay green.
- [ ] P5R-T07 Normalize core lifecycle naming and responsibilities. Acceptance: lifecycle coordination types are named by their actual role, startup/rollback/fatal-shutdown semantics remain unchanged, and any public rename follows wiki/review requirements in the active Issue.
- [ ] P5R-T08 Audit core configuration, logging, timing, and native-resource naming. Acceptance: ambiguous names are replaced with responsibility-revealing names where justified, already-clear names are explicitly retained, and behavior/ownership contracts do not change.
- [ ] P5R-T09 Audit spatial/math naming and structure. Acceptance: camera, transform, geometry, screen-ray, and quantization APIs read consistently with D-041/D-045/D-046/D-047; no coordinate, unit, tolerance, allocation, or serialization semantic changes.
- [ ] P5R-T10 Rename/decompose the fixed reference-scene renderer path. Acceptance: the current `IndexedStaticMeshPipeline` responsibility is represented by a name that describes the reference-room/world rendering role, room fixture mesh/texture construction is separated where it improves clarity, and Phase 5 rendering output remains unchanged.
- [ ] P5R-T11 Split renderer frame orchestration by responsibility. Acceptance: frame-uniform preparation, visibility/culling, draw execution, diagnostics publication, and related internal responsibilities have explicit owners without changing the public `OpenGlRenderer` contract or draw semantics.
- [ ] P5R-T12 Normalize renderer material/submission/culling/light internal names. Acceptance: material descriptors/state policy, world draw submission/sorting, light selection, and culling names communicate their function without changing ordering, state, limits, or shader behavior.
- [ ] P5R-T13 Normalize uniform-block/color/presentation naming. Acceptance: uniform-block classes are named for the data they actually carry, sRGB/presentation responsibilities are explicit, shader ABI/binding/layout and D-056/D-063 output semantics remain unchanged.
- [ ] P5R-T14 Refactor OpenGL resource/backend ownership internals. Acceptance: buffer/texture/program/VAO/sampler/framebuffer/resource ownership and backend adapter names are consistent, cleanup duplication is reduced only where behavior is preserved, and native leak/thread-affinity regressions remain green.
- [ ] P5R-T15 Refactor debug-geometry and view-model internals. Acceptance: packing/render/projection responsibilities and names are clear, debug/view-model behavior remains identical, and no new public gameplay/view-model API appears.
- [ ] P5R-T16 Decompose `SandboxMain` into a small entry point plus clearly named application-loop/input/scene/diagnostic responsibilities. Acceptance: the persistent sandbox retains all current controls and observable behavior while its main class no longer owns unrelated responsibilities.
- [ ] P5R-T17 Clean sandbox naming and legacy compatibility surface. Acceptance: sandbox control/diagnostic/framebuffer/helper names communicate intent, obsolete compatibility entry points are removed only after repository-reference verification, and canonical `runSandbox` behavior remains unchanged.
- [ ] P5R-T18 Decompose the standalone renderer visual demo. Acceptance: entry-point, animated-light, material-comparison, and cleanup responsibilities have clear names and the demo remains isolated from production/public API behavior.
- [ ] P5R-T19 Normalize client/server bootstrap and version-report naming. Acceptance: executable/bootstrap/reporting responsibilities are clear, shared compatibility output remains identical, and no new client/server dependency edge is introduced merely to remove small duplication.
- [ ] P5R-T20 Audit feasibility-spike naming/isolation. Acceptance: spike/harness names clearly identify experimental purpose, production code does not begin depending on spike packages, and no spike is promoted into architecture by refactor accident.
- [ ] P5R-T21 Perform the deliberate public-API naming pass after internals stabilize. Acceptance: every public P1-P5 type/member is reviewed for clarity; justified renames are synchronized across consumers/tests/wiki and independently reviewed; already-clear public names remain stable.
- [ ] P5R-T22 Synchronize test/fixture/evidence naming with the refactored production vocabulary. Acceptance: test names communicate the behavior they protect, historical CI/evidence identifiers are preserved where automation depends on them, and no test is weakened merely to simplify a rename.
- [ ] P5R-T23 Reorganize internal packages only where stable responsibility groups are now evident. Acceptance: package names reflect domain responsibility, public packages/module edges stay within accepted architecture, and moves do not create cyclic/lower-to-higher dependencies.
- [ ] P5R-T24 Perform a pattern/scalability hardening pass based on observed code. Acceptance: any introduced/strengthened Facade, Adapter, Strategy/Policy, Command, composition/DI, or similar pattern cites the concrete problem it solves; speculative Singleton/Service-Locator/Object-Pool/ECS work is rejected unless separately justified by current evidence.
- [ ] P5R-T25 Run the final consistency/dead-code/stale-name cleanup. Acceptance: repository search finds no unintended stale renamed symbols, stale Javadocs/wiki/examples, avoidable compatibility aliases, or unused refactor scaffolding; documentation and sandbox guidance match the final code.
- [ ] P5R-T26 Execute the Phase 5R exit review. Acceptance: required P5R tasks are accepted; full automated verification is green on the final candidate/merge path; the persistent sandbox and standalone renderer visual demo preserve their accepted behavior; public/internal/module/ownership/spatial/persisted/protocol contracts are reconciled; and the review explicitly records PASS before P6-T01 may be materialized.

Exit gate: the completed P1-P5 codebase passes the full repository verification and owner-facing regression review after responsibility decomposition and naming normalization, with no unintended behavior/module/ownership/spatial/format changes, no unresolved stale names/docs, and a recorded Phase 5R PASS. **Phase 6 is blocked until this gate passes.**
