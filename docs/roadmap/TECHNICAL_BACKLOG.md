...
- [ ] P5R-T18 Decompose the standalone renderer visual demo. Active via Issue #278: candidate keeps `RendererVisualDemo` / `runRendererVisualDemo` unchanged while extracting package-private application lifecycle, frame-loop/framebuffer state, animated lighting, and material-comparison GL ownership; acceptance remains clear responsibilities with unchanged production/public behavior.
