...
- [ ] P6-T11 Implement `ResourceHandle<T>` with loading, ready, failed, and released states. Acceptance: gameplay cannot obtain an untyped native handle. Active executable contract: Issue #393 from `master` `f3367cdcfb8f1d72ffdbe3d5ce6c7aba05e5f4e2`; public gameplay surface is typed/read-only except idempotent local release, while package-private completion owns LOADING -> READY/FAILED and no loader/cache/reference counting/native handle is introduced.
