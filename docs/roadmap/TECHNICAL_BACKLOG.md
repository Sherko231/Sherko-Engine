...
- [ ] P5R-T19 Normalize client/server bootstrap and version-report naming. Active via Issue #279: keep `ClientMain` / `ServerMain` and their Gradle entry tasks unchanged; rename only internal `VersionReport` helpers to `ClientVersionReport` / `ServerVersionReport`; preserve exact compatibility output and existing dependency/headless boundaries.
