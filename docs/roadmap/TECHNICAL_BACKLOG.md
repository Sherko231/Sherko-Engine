...
- [ ] P5R-T20 Audit feasibility-spike naming/isolation. Active via Issue #280: rename only `IntegratedNativeSoakSpike` -> `IntegratedNativeEvidenceHarness`, KEEP the other reviewed spike/harness names, preserve historical task/evidence behavior, and add a Gradle isolation check that rejects project dependencies on `:feasibility-spikes` from other subprojects.
