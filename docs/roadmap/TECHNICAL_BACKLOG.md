...
- [ ] P5R-T22 Synchronize test/fixture/evidence naming with the refactored production vocabulary. Active via Issue #302: all 99 test classes/method names reviewed; rename only the generic invalid GLSL fixture `broken.frag` -> `invalid-syntax.frag` and coupled method wording; preserve historical CI/evidence identifiers.
