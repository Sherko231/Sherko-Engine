...
- [ ] P5R-T23 Reorganize internal packages only where stable responsibility groups are now evident. Active via Issue #303: fresh audit finds no justified move; renderer internals form one package-private connected component, platform helpers must remain colocated with public facades to avoid visibility widening, visual-demo internals depend on package-private renderer infrastructure, and client/server internal roots are single-helper packages.
