...
- [ ] P6-T09 Cook mono/stereo audio metadata and Vorbis payloads with validated sample rate/channel count. Acceptance: a malformed source is rejected by the cooker. Active executable contract: Issue #386 from `master` `10501b1b5b548a46c715069abc23e6cef7393341`; AUDIO `.ogg` sources are validated offline through stb_vorbis and persisted as exact compressed payloads inside validated `SAUD` v1, while P6-T10+ remain out of scope.
