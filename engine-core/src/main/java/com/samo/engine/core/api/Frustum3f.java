...
public final class Frustum3f {
    ...
    public boolean containsPoint(Vector3fc point) {
        ...
        return left.signedDistance(point) >= 0.0f
                && right.signedDistance(point) >= 0.0f
                && bottom.signedDistance(point) >= 0.0f
                && top.signedDistance(point) >= 0.0f
                && near.signedDistance(point) >= 0.0f
                && far.signedDistance(point) >= 0.0f;
    }
    ...
    public boolean intersects(Sphere3f sphere) {
        ...
        return left.signedDistance(x, y, z) >= -radius
                && right.signedDistance(x, y, z) >= -radius
                && bottom.signedDistance(x, y, z) >= -radius
                && top.signedDistance(x, y, z) >= -radius
                && near.signedDistance(x, y, z) >= -radius
                && far.signedDistance(x, y, z) >= -radius;
    }

    public boolean intersects(Aabb3f aabb) {
        Objects.requireNonNull(aabb, "aabb");
        return notOutside(left, aabb)
                && notOutside(right, aabb)
                && notOutside(bottom, aabb)
                && notOutside(top, aabb)
                && notOutside(near, aabb)
                && notOutside(far, aabb);
    }
