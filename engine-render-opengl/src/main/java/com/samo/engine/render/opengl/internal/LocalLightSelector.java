    ...
    LocalLightSelector(EngineLogger logger, int maximum) {
        ...
        if (maximum < 1 || maximum > SHADER_CAPACITY) {
            throw new IllegalArgumentException(
                    "maxLocalLights must be within [1," + SHADER_CAPACITY + "]");
        }
    ...
    List<RenderLocalLight> select(List<RenderLocalLight> submitted) {
        ...
        if (submittedCount > maximum) {
            ...
            logger.log(
                    ...
                    "Local light limit exceeded: submitted=%d accepted=%d dropped=%d configuredMax=%d"
                            .formatted(submittedCount, acceptedCount, droppedCount, maximum),
