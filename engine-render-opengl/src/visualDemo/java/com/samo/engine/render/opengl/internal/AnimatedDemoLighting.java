...
final class AnimatedDemoLighting {
    ...
    static LightingFrame at(double seconds) {
        ...
        Vector3f spotDirection = new Vector3f(0.0f, 0.0f, -2.25f).sub(spotPosition).normalize();

        RenderPointLight point = new RenderPointLight(pointX, pointY, pointZ, 1.0f, 0.28f, 0.06f, 1.0f, 4.5f);
        RenderSpotLight spot = new RenderSpotLight(spotX, spotY, spotZ, spotDirection.x, spotDirection.y, spotDirection.z, 0.10f, 0.60f, 1.0f, 1.0f, 5.0f,
            (float) Math.toRadians(12.0), (float) Math.toRadians(28.0));
