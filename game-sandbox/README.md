...
- public `OpenGlRenderer` production composition drawing the fixed renderer-owned P5-T18 room fixture once across the full framebuffer through the opaque baseline material; the room contains mapped UVs, a non-uniform internal sRGB texture, multiple depth-separated surfaces, and a nearer occlusion panel while preserving one presentation encode;
