...
final class SandboxSceneSetup {
    ...
    RenderFramePacket frame(SandboxCamera camera, SandboxFramebufferSize framebufferSize, long cumulativeTicks, long inputFrameId, MaterialAsset material,
        ...
        DebugFrame debugFrame = new DebugFrame(visualLab, List.of(new DebugTextCounter("simulation/tick", cumulativeTicks), new DebugTextCounter("input/frame", inputFrameId)));
    ...
    static List<RenderLocalLight> sandboxLocalLights(MaterialAsset material) {
        ...
        RenderSpotLight materialSpot = new RenderSpotLight(1.10f, 0.75f, -0.75f, -1.10f, -0.55f, -0.55f, palette.spotRed(), palette.spotGreen(), palette.spotBlue(), 0.40f, 4.5f,
            0.28f, 0.70f);
