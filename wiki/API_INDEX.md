...
| `DebugColor` | Immutable finite linear RGB debug color in `[0,1]`. |
| `DebugPrimitive` | Sealed renderer-neutral geometry family permitting line, AABB, sphere, and finite ray submissions. |
| `DebugLine` | Immutable D-041 world-space line segment with debug color. |
| `DebugAabb` | Immutable wrapper pairing accepted `Aabb3f` geometry with debug color. |
| `DebugSphere` | Immutable wrapper pairing accepted `Sphere3f` geometry with debug color. |
| `DebugRay` | Immutable finite-length visualization of accepted normalized `Ray3f` with debug color. |
| `DebugTextCounter` | Bounded ASCII diagnostic label plus signed `long` value; not general text/UI. |
| `DebugFrame` | Immutable per-frame debug snapshot limited to 64 geometry primitives and 16 text counters. |
...
| Type | Purpose |
| --- | --- |
| `RenderLocalLight` | Sealed renderer-submission family for bounded per-frame local lights; currently permits point and spot values only. |
| `RenderPointLight` | Immutable D-041 world-space point light with linear RGB/intensity and finite positive range. |
| `RenderSpotLight` | Immutable D-041 world-space spot light with normalized ray-travel direction, linear RGB/intensity, finite positive range, and inner/outer cone radians. |
| `RenderFramePacket` | Immutable per-frame renderer submission snapshot that defensively copies view/projection matrices and local-light order and carries one immutable bounded `DebugFrame`; owns no native resources and retains no mutable world/gameplay objects. |
| `RenderCullingCounters` | Immutable latest-successful-frame CPU culling diagnostics: tested candidates, visible candidates, culled candidates, and submitted draws. |
| `OpenGlRenderer` | Bounded production OpenGL renderer composition: renders the current reference scene plus bounded per-frame debug geometry, consumes bounded point/spot submissions, and exposes latest-successful culling and debug text-counter snapshots. |

`OpenGlRenderer.create(window.openGlThreadGuard(), nativeResources)` requires an already-started production OpenGL context on the owner thread. `new RenderFramePacket(view, projection, framebufferWidth, framebufferHeight)` snapshots the caller's D-041/D-045 matrices and positive framebuffer pixel dimensions. `render(RenderFramePacket)` consumes that immutable snapshot synchronously without retaining it. The prior matrix/size overload remains available and constructs an equivalent packet. Before the current fixed world draw candidate, the renderer derives the active view frustum from the packet matrices and tests the renderer-owned world AABB through the accepted P4 `Frustum3f.intersects(Aabb3f)` semantics. `lastCullingCounters()` returns immutable culling diagnostics from the latest successful render only; `lastDebugTextCounters()` similarly returns the latest successful bounded text-counter snapshot. A failed render publishes neither partial diagnostic set. The current fixed renderer path samples the internal non-uniform sRGB room texture in linear space and draws one full-frame room through the opaque baseline material. Alternate internal material/state behavior remains regression-tested without being forced into the normal runtime presentation. Presentation still receives exactly one sRGB encode: hardware `GL_FRAMEBUFFER_SRGB` when the actual default buffer is sRGB, or the bounded internal fragment fallback when it is linear. The internal material model and P5-T13 directional light remain implementation details. P5-T14 publicly exposes only bounded per-frame point/spot submission values; the renderer still exposes no native handles, arbitrary mesh/texture/public-material API, shadow resources, asset loading, or world/ECS light ownership/submission.
