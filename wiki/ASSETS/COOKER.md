...
- resource caches, reference counting, runtime loading for non-MESH asset types, or hot reload;
- renderer/world/editor integration.

P6-T13 now consumes the cooker's unchanged manifest v1 and SMES v1 output through the public asynchronous MESH loader. The cooker still does not create runtime handles, mutate an existing cache, or define runtime loading for the other asset types.
