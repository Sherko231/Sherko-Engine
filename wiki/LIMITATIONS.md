...
P5-T07 provides the first bounded public production renderer path through `OpenGlRenderer` plus window-owned `GlfwWindow.present()`. P5-T08 adds one internal sRGB reference texture, explicit sRGB-versus-linear storage rules, and one default-framebuffer sRGB encode boundary. P5-T09 now reuses the same engine-owned indexed position-only mesh twice through two internal immutable material values carrying explicit shader/textures/scalars/blend/depth/cull policy.

This is intentionally not a general renderer API yet. The P5-T09 material value is package-internal because P6 has not defined stable runtime asset/resource references. There is still no arbitrary mesh submission, public/arbitrary texture creation, public material creation, asset loading/import, lighting, HDR/tonemapping/fog/post-processing, world/ECS integration, batching, frustum culling/sorting, render graph, render worker/command queue, or public native resource handle surface. The persistent sandbox demonstrates the fixed baseline/tinted two-material reference scene while preserving the existing input/window/timing playground.
