...
This is intentionally not a general renderer API yet. The P5-T09 material value is package-internal because P6 has not defined stable runtime asset/resource references. The packet carries frame/camera data plus bounded local-light values only; it still does not invent mesh/material/asset/native resource identities or world/ECS ownership before later phases. P5-T11 culling is limited to the renderer-owned reference AABB/candidate set; it is not a general world extraction or spatial acceleration API. There is still no arbitrary mesh submission, public/arbitrary texture creation, public material creation, asset loading/import, directional-light replacement, more than eight local lights per frame, unbounded/dynamic GPU light storage, shadows, PBR/IBL, HDR/tonemapping/fog/post-processing, world/ECS light ownership/integration, batching/instancing, BVH/octree/broad phase, occlusion/GPU culling, GPU-driven sorting, OIT, clustered/Forward+ lighting, render graph/pass scheduler, render worker/command queue, or public native resource handle surface. The persistent sandbox demonstrates the fixed baseline/tinted two-material reference scene while preserving the existing input/window/timing playground.
