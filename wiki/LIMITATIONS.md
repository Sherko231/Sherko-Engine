...
P5-T07 provides the first bounded public production renderer path through `OpenGlRenderer` plus window-owned `GlfwWindow.present()`. P5-T08 adds one internal sRGB reference texture, explicit sRGB-versus-linear storage rules, and one default-framebuffer sRGB encode boundary. P5-T09 reuses the same engine-owned indexed mesh twice through two internal immutable material values carrying explicit shader/textures/scalars/blend/depth/cull policy. P5-T18 evolves only that internal validation fixture into a mapped-UV textured room with multiple depth-separated surfaces and a nearer occlusion panel; it does not create a public mesh/texture/material/resource API. P5-T10 adds public immutable `RenderFramePacket`, which snapshots finite camera matrices and framebuffer pixel dimensions before synchronous renderer consumption without retaining mutable caller/world objects. P5-T11 adds a linear CPU candidate scan over the current fixed reference scene: a world AABB is tested against a frustum extracted from the active frame, with inclusive contact semantics inherited from P4 geometry and immutable latest-success culling counters exposed for diagnostics. P5-T12 then sorts only the visible internal candidates deterministically: opaque submissions by renderer-owned logical program/material/mesh keys, transparent submissions back-to-front by camera-space depth with stable key/sequence ties. P5-T13 adds one fixed renderer-owned unshadowed directional light over that same reference scene, with explicit D-041 ray-travel direction and linear color/intensity semantics; it does not add a public light-submission contract. P5-T14 adds public immutable point/spot light submission values through the frame packet with one combined configurable maximum from 1–8; overflow deterministically keeps the first N values and emits one renderer warning. P5-T16 adds bounded renderer-neutral per-frame debug line/AABB/sphere/ray values in `engine-core`, a fixed-capacity OpenGL line adapter, and bounded label/value text-counter publication; it does not add a retained debug scene or font/UI renderer. P5-T17 adds only one internal engine-owned camera-relative view-model validation fixture with isolated depth and explicit projection; it does not add a public view-model mesh/material/asset submission API or gameplay object.

This is intentionally not a general renderer API yet. The P5-T09 material value is package-internal because P6 has not defined stable runtime asset/resource references. The packet carries frame/camera data plus bounded local-light values only; it still does not invent mesh/material/asset/native resource identities or world/ECS ownership before later phases. P5-T11 culling is limited to the renderer-owned reference AABB/candidate set; it is not a general world extraction or spatial acceleration API. There is still no arbitrary mesh submission, public/arbitrary texture creation, public material creation, public view-model mesh/material submission, asset loading/import, gameplay hands/weapons/tools, skeletal animation/IK, third-person presentation, directional-light replacement, more than eight local lights per frame, unbounded/dynamic GPU light storage, shadows, PBR/IBL, HDR, tonemapping, fog, bloom, exposure, color grading, general post-processing, offscreen framebuffer pipeline, world/ECS light ownership/integration, retained/persistent debug scene, debug IDs or multi-frame debug lifetimes, font/text-glyph debug rendering, editor/ImGui runtime HUD, batching/instancing, BVH/octree/broad phase, occlusion/GPU culling, GPU-driven sorting, OIT, clustered/Forward+ lighting, render graph/pass scheduler, render worker/command queue, public presentation settings, or public native resource handle surface. The persistent sandbox demonstrates the fixed textured-room validation scene through the same baseline/tinted internal material path and now drives its rendered D-041/D-045 camera from W/A/S/D plus mouse LOOK, while preserving the existing input/window/timing playground.

## Assets/world/physics/audio/networking/editor

`engine-assets` now exposes P6-T01 `AssetId`, the P6-T02 strict source metadata API, the P6-T11 generic `ResourceHandle<T>` lifecycle boundary, and P6-T12 structured `AssetLoadError` diagnostics for deterministic missing-content fallback resolution. P6-T03 adds the offline cooker/manifest/fresh-cache foundation. P6-T04 adds package-internal Assimp import for MESH `.gltf` / `.glb` sources and records its intrinsic first-use vertex remapping/UV-origin normalization. P6-T05 converts imported mesh spatial data exactly once into D-041 engine space. P6-T06 enables only Assimp tangent-space calculation, preserves per-vertex tangent handedness, and rejects normal-mapped meshes missing required UV0/normals before output creation. P6-T07 persists MESH assets as validated little-endian `SMES` schema v1 with explicit counts, engine-space bounds, interleaved vertex/index streams, and CRC32C. P6-T08 decodes PNG/JPEG TEXTURE sources only in the offline cooker, emits deterministic RGBA8 mip chains in validated little-endian `STEX` schema v1, and adds no color-space/usage semantics or runtime texture API. P6-T09 validates mono/stereo Ogg Vorbis AUDIO sources only in the offline cooker and persists their exact compressed payload plus channel/sample-rate metadata in validated little-endian `SAUD` schema v1; it adds no OpenAL playback or public runtime audio/resource API. P6-T10 adds strict MATERIAL/PREFAB/SCENE dependency sidecars, a package-private deterministic AssetId/logical-shader dependency graph, reverse transitive invalidation queries, and persisted `dependencies.json` without changing source metadata v1 or adding a SHADER AssetType. P6-T11 handles expose only AssetId, LOADING/READY/FAILED/RELEASED state, READY-only typed Java values, and idempotent local release; they expose no native/backend identifier and provide no public completion method. Other asset payloads remain source pass-through. Metadata writing/migration, broader importer settings, normal/UV generation, runtime audio decode/playback, incremental cache mutation/recooking, resource caches/reference counting, texture/audio/material runtime loading, arbitrary renderer/world asset submission, and hot reload remain unimplemented. P6-T12 provides deterministic fallback values/errors. P6-T13 adds strict runtime manifest-v1 loading plus asynchronous MESH read/SMES decode into public `MeshAsset` values and a package-private D-049-guarded OpenGL vertex/index buffer uploader. It does not expose those GPU buffers publicly or draw arbitrary meshes. The other target modules in this section still exist according to the repository architecture, but module existence does not mean a consumer API has been implemented. Do not write wiki examples for planned APIs until the corresponding implementation task is complete and verified.
