...
`engine-assets` now exposes P6-T01 `AssetId` plus the P6-T02 strict source metadata API: `AssetType`, immutable `SourceAssetMetadata`, and `SourceAssetMetadataLoadException`. P6-T03 adds an offline internal command-line cooker that discovers adjacent `<source>.asset.json` files, requires a fresh output cache, writes deterministic `manifest.json` schema v1, and emits nonzero `assets/<AssetId>.bin` files as temporary byte-for-byte source copies. Metadata writing/migration, type-specific importer settings, Assimp/glTF/image/audio decoding, final cooked binary schemas/checksums, dependency graphs/incremental recooking, runtime manifest/resource handles/caches, reference counting, runtime loading, and renderer/world asset submission remain unimplemented. The other target modules in this section still exist according to the repository architecture, but module existence does not mean a consumer API has been implemented. Do not write wiki examples for planned APIs until the corresponding implementation task is complete and verified.
