...
`engine-assets` now exposes P6-T01 `AssetId` plus the P6-T02 strict source metadata API: `AssetType`, immutable `SourceAssetMetadata`, and `SourceAssetMetadataLoadException`. P6-T03 adds the offline cooker/manifest/fresh-cache foundation. P6-T04 adds package-internal Assimp import for MESH `.gltf` / `.glb` sources during pre-output validation, copying positions/normals/tangent xyz/UV0/triangle indices into Java-owned Assimp import-basis values; indexed glTF vertices are intrinsically remapped by first use and UV V is intrinsically normalized by Assimp, with no additional post-process spatial conversion and releasing Assimp scene memory before return. The persisted `.bin` payload is still the original source bytes. Metadata writing/migration, type-specific importer settings beyond this fixed glTF boundary, source-to-engine coordinate/unit conversion, tangent generation/required-UV policy, final cooked binary schemas/checksums, image/audio decoding, dependency graphs/incremental recooking, runtime manifest/resource handles/caches, reference counting, runtime loading, and renderer/world asset submission remain unimplemented. The other target modules in this section still exist according to the repository architecture, but module existence does not mean a consumer API has been implemented. Do not write wiki examples for planned APIs until the corresponding implementation task is complete and verified.
